...
struct QueryFilter {
    ...
    QueryFilter(
        ...
        bool is_and
    ) : _filters(filters), _is_and(is_and) {};
...
struct QueryFilterHash
{
  std::size_t operator()(const QueryFilter& k) const
  {
    ...
    int sum = 0;
    for (const auto& elem : k._filters) {
        sum += elem;
    } 
    if (k._is_and) {
        return sum;
...
std::vector<uint32_t> join(std::vector<uint32_t> a, std::vector<uint32_t> b){
