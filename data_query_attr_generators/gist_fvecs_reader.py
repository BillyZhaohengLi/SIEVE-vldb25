...
if __name__ == "__main__":
    query_matrix = fvecs_read("~/bigann/biganntest/data/gist/gist_query.fvecs", np.int32)
    dataset = fvecs_read("~/bigann/biganntest/data/gist/gist_base.fvecs", np.int32)
    ...
    # pickle.dump(np.transpose(data_filters_matrix), open("/home/zl20/bigann/biganntest/data/gist/gist_data_attrs.pkl", "wb"))
    ...
    while len(q_list) < sum:
        ...
        template = [np.random.normal(loc = 0, scale = 0.12) for i in range(4)]
        template.sort()
    ...
    pickle.dump(tmp_matrix, open("~/bigann/biganntest/data/gist_another/gist_query_attrs.pkl", "wb"))
